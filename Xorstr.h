...
//Use This For Shit Like Anti Debugging & More
...
Usage:
Pass string literals into the XorStr macro to obfuscate them at compile
time. XorStr returns a reference to an ay::obfuscated_data object with the
following traits:
...
Example:
const char* obfuscated_string = XorStr("Hello World");
std::cout << obfuscated_string << std::endl;
----------------------------------------------------------------------------- */
//
#pragma once
#ifndef XorStr_DEFAULT_KEY
// The default 64 bit key to obfuscate strings with.
// This can be user specified by defining XorStr_DEFAULT_KEY before
// including obfuscate.h
#define XorStr_DEFAULT_KEY 0x5AA5D2B4D39B2B69ull
#endif
...
namespace ay
{
	...
	// array 'data'
	template <size_type N, key_type KEY = XorStr_DEFAULT_KEY>
...
#define XorStr(data) XorStr_KEY(data, XorStr_DEFAULT_KEY)
